...
class AssisSubController extends Controller
{
    public function show(Request $request)
    {
        ...
        $search = DB::table('registro_casos as rc')
            ...
            ->join('registro_casos_subsidio as rcal', 'rca.id', '=', 'rcal.fk_registro_casos_asistencia')
            ...
            'rcal.id', 'rcal.comuna', 'rcal.integrantes', 'rcal.direccion', 'rcal.gestion', 'rcal.envia_a_as', 'rcal.envia_a_pf', 'rcal.aprobado', 'rcal.nro_pr', 'rcal.fecha_entrega', 'rcal.pagado', 'rcal.fecha');
        //count
        $count = $search->count();
    ...
    public function showById($id)
    {
        $search = DB::table('registro_casos as rc')
            ...
            ->join('registro_casos_subsidio as rcal', 'rca.id', '=', 'rcal.fk_registro_casos_asistencia')
            ->select('rc.id as id_caso', 'd.nombres', 'd.apellidos', 'd.nacionalidad', 'dta.id as id_asistencia', 'dta.descripcion as asistencia', 'rca.id as id_rasis',
            'rcal.id', 'rcal.comuna', 'rcal.integrantes', 'rcal.direccion', 'rcal.gestion', 'rcal.envia_a_as', 'rcal.envia_a_pf', 'rcal.aprobado', 'rcal.nro_pr', 'rcal.fecha_entrega', 'rcal.pagado', 'rcal.fecha', 'rcal.fecha_entrega as fecha2', 'rcal.fecha as fecha1')
            ->where('rc.id', '=', $id);
        //count
        $count = $search->count();
        //query
    ...
    public function createById(Request $request, $id)
    {
        ...
        try {
            ...
            $assis->fk_data_tipo_asistencia = 3;
            $assis->save();
            //cases status model
            ...
            $assisStatus->id_user = intval($request->input('v6'));
            $assisStatus->save();
            //model assis
            $sub = new AssisSub;
            $sub->comuna = strtoupper($request->input('v1'));
            $sub->integrantes = intval($request->input('v2'));
            $sub->direccion = strtoupper($request->input('v3'));
            $sub->gestion = strtoupper($request->input('v4'));
            $sub->fecha = $request->input('v5');
            $sub->fk_registro_casos_asistencia = $assis->id;
            $sub->save();
            //return successful response
            return response()->json(['case' => $case, 'assis' => $assis, 'sub' => $sub, 'message' => 'CREATED'], 201);
            //end
        } catch (\Exception $e) {
            //return error message
            return response()->json(['message' => 'Sub Created Failed!'], 409);
        }
    }
    ...
    public function updateById(Request $request, $id)
    {
        ...
        try {
            //find
            $sub = AssisSub::findOrFail($id);
            $sub->update([
                'comuna' => strtoupper($request->input('v1')),
                'integrantes' => intval($request->input('v2')),
                'direccion' => strtoupper($request->input('v3')),
                'gestion' => strtoupper($request->input('v4')),
                'envia_a_as' => strtoupper($request->input('v5')),
                'envia_a_pf' => strtoupper($request->input('v6')),
                'aprobado' => strtoupper($request->input('v7')),
                'nro_pr' => strtoupper($request->input('v8')),
                'fecha_entrega' => $request->input('v9'),
                'pagado' => strtoupper($request->input('v10')),
                'fecha' => $request->input('v11')
            ]);
            //return successful response
            return response()->json(['Sub' => $sub, 'message' => 'UPDATED'], 201);
            //end
        } catch (\Exception $e) {
            //return error message
