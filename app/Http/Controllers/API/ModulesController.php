...
class ModulesController extends Controller
{
    ...
    public function show(Request $request)
    {
        ...
        if ($request->has('role') and !empty($request->has('role')) ) {
            $search->where('role', 'like', '%' . $request->input('role') . '%');
        }
        if ($request->has('status') and !empty($request->has('status'))) {
            $search->where('apellidos', 'like', '%' . $request->input('apellido') . '%');
        }
        if ($request->has('documento') and !empty($request->has('documento'))) {
            $search->where('documento_numero', 'like', '%' . $request->input('documento') . '%');
        }
        if ($request->has('identidad') and !empty($request->has('identidad'))) {
            $search->where('identidad_numero', 'like', '%' . $request->input('identidad') . '%');
        }
        if ($request->has('genero') and !empty($request->has('genero'))) {
